...
export const formulas: FormulaSubject[] = [
    {
        ...
        topics: [
            ...
            },
            {
                ...
                concepts: [
                    { title: 'Ohm\'s Law', formula: 'V = IR', derivation: 'An empirical law stating that voltage is proportional to current for many materials.' },
                    { title: 'Resistance & Resistivity', formula: 'R = ρ(L/A)', derivation: 'Resistance (R) depends on the material\'s resistivity (ρ) and its geometry (length L, area A).' },
                    { title: 'Kirchhoff\'s Junction Rule (KCL)', formula: 'ΣI_in = ΣI_out', derivation: 'Based on the conservation of electric charge at any junction in a circuit.' },
                    { title: 'Kirchhoff\'s Loop Rule (KVL)', formula: 'ΣV = 0 for any closed loop', derivation: 'Based on the conservation of energy in a circuit. The sum of potential rises and drops around any closed loop is zero.' }
                ]
            },
            {
                name: 'Magnetic Effects',
                concepts: [
                    { title: 'Lorentz Force', formula: 'F = q(E + v x B)', derivation: 'A fundamental law describing the total force on a charge q moving with velocity v in electric field E and magnetic field B.' },
                    { title: 'Biot-Savart Law', formula: 'dB = (μ₀/4π) * (I dL x r̂) / r²', derivation: 'A fundamental law of magnetostatics that describes the magnetic field generated by a small segment of a current-carrying wire.' },
                    { title: 'Ampere\'s Law', formula: '∮ B · dL = μ₀I_enclosed', derivation: 'Relates the integrated magnetic field around a closed loop to the electric current passing through the loop. Useful for symmetric current distributions.' }
                ]
            },
             {
    ...
    },
    {
        ...
        topics: [
            ...
            },
            {
                ...
                concepts: [
                    { title: 'Kp and Kc Relation', formula: 'K_p = K_c(RT)^Δn_g', derivation: 'Derived from the ideal gas law (P = CRT) by substituting partial pressures for concentrations in the equilibrium expression.' },
                    { title: 'Gibbs Energy and Equilibrium', formula: 'ΔG° = -RT ln(K)', derivation: 'A fundamental equation linking the standard Gibbs free energy change to the equilibrium constant.' },
                    { title: 'Henderson-Hasselbalch Equation', formula: 'pH = pKa + log([A⁻]/[HA])', derivation: 'Derived from the acid dissociation constant (Ka) expression, it is used to calculate the pH of a buffer solution.' },
                    { title: 'Solubility Product (Ksp)', formula: 'For AxBy ⇌ xA⁺ + yB⁻, Ksp = [A⁺]^x[B⁻]^y', derivation: 'The equilibrium constant for the dissolution of a sparingly soluble salt.' }
                ]
            },
            {
                name: 'Solutions',
                concepts: [
                    { title: 'Raoult\'s Law', formula: 'P_A = P°_A * x_A', derivation: 'An empirical law defining an ideal solution, where the partial vapor pressure of a component is proportional to its mole fraction.' },
                    { title: 'Boiling Point Elevation', formula: 'ΔT_b = i * K_b * m', derivation: 'A colligative property derived from the lowering of vapor pressure by a non-volatile solute.' },
                    { title: 'Freezing Point Depression', formula: 'ΔT_f = i * K_f * m', derivation: 'A colligative property where a solute disrupts the crystal lattice formation of the solvent.' },
                    { title: 'Osmotic Pressure', formula: 'π = i * MRT', derivation: 'The pressure required to prevent osmosis, analogous to the ideal gas law.' }
                ]
            }
        ]
    },
